...
-- describing tables
desc db1.client_master;
desc db1.product_master;
desc db1.salesman_master;
desc db1.sale_order;
desc db1.sale_order_details;
...
drop table db1.product_master;
drop table db1.salesman_master;
drop table db1.sale_order;
drop table db1.sale_order_details;

-- deleting the data inside a table
truncate table db1.client_master;
truncate table db1.product_master;
truncate table db1.salesman_master;
truncate table db1.sale_order;
truncate table db1.sale_order_details;


-- 3) Exercises computation on table data:
-- a):- Find out the clients who stay in a city whose second letter is ‘a’.
	SELECT * FROM db1.client_master WHERE NAME like "_a%";
            
-- b) Find out the clients who stay in a city whose second letter is ‘a’. 
	SELECT * FROM db1.client_master WHERE City LIKE "_a%";

-- c) Find the list of all clients who stay in ‘Bombay’ or ‘Delhi’.
	SELECT *FROM db1.client_master WHERE CITY LIKE "Bombay" OR "Delhi";
        
-- d) Print the list of clients whose bal_due is greater than value 10000.
	select * from db1.client_master where (Bal_due >=10000);

-- e) Print the information from sales_order table for order placed in the month of January.
	select * from db1.sale_order where (order_date like '%-_1-%');

-- f) Display the order information for client_no ‘C00001’ and ‘C00002’.
	select * from db1.sale_order where  (Client_no='C00001' or Client_no='C00002');
...
-- above query as new_price.
	SELECT Product_no , Sell_price * .15 AS "New_price" FROM db1.product_master where (Sell_price > 1500  );

-- i) List the names, city and state of clients who are not in the state of ‘Maharashtra’.
	select Name,City,State from db1.client_master where (State not like 'Maharashtra');
...
-- l) Determine the maximum and minimum product prices. Rename the output as max_price and min_price respectively.
	SELECT * from db1.product_master;
	SELECT min(new_price) AS "min_price" , max(new_price) AS "max_price" from db1.product_master ;
...
-- a) Display the order number and day on which clients placed their order.
	select order_no ,order_date, dayname(order_date) from db1.sale_order ;
 ...
 -- b) Display the month (in alphabets) and date when the order must be delivered.       
	SELECT order_date AS "Ordered when : " ,day(Dely_date),  
	   month(Dely_date),monthname(Dely_date) FROM db1.sale_order ORDER BY (Dely_date);
...
-- c) Print information from client_master, product_master, sales_order tables in the following format for all the records: {cust_name} has placed order {order_no} on {order_date}.
